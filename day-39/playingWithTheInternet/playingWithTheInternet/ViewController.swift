...
let updateDictionary = ["bio": "Hello from AlamoFire"]
        let authHeaders = ["Authorization": "token f3ec17d78b55e43792eaefcfa7df2c8c66c95981"]
        Alamofire.request(.PATCH,
                          "https://api.github.com/user",
                          parameters: updateDictionary,
                          encoding: ParameterEncoding.JSON,
                          headers: authHeaders)
            .responseJSON { (response) in
                if let value=response.result.value {
                    let json = JSON(value)
                    let privateRepos = json["plan"]["private_repos"].intValue
                    print(json)
                }
